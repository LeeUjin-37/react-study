...
const FoodContainer = () => {
  ...
  return (
    <>
      <div>
        <input type="text" placeholder='한국 음식을 입력하세요' ref={(el) => inputRef.current[0] = el} />
        <input type="text" placeholder='영어명을 입력하세요' ref={(el) => inputRef.current[1] = el} />
        <button onClick={handleAddFoodOnClick}>추가</button>
        <button onClick={handleRemoveFoodOnClick}>삭제</button>
      </div>
      <ul>
        {foodList}
      </ul>
